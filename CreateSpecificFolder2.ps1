...
# => 移動元PC入力  
$mSrcPC = Read-Host "交換元PCを入力してください"
$rSrcPC = Get-ADComputer -Filter "name -eq '$($mSrcPC)"
If($rSrcPC -eq $null)
{
    Write-Warning "入力した交換元PCは存在しません。"
    exit
}
Write-Host "`n"
...
# => 移動先PC入力  
$mTrgPC = Read-Host "交換先PCを入力してください"
$rTrgPC = Get-ADComputer -Filter "name -eq '$($mTrgPC)"
If($rTrgPC -eq $null)
{
    Write-Warning "入力した交換先PCは存在しません。"
    exit
}
Write-Host "`n"
...
$mRemotePrivateDir = "$($rRemoteDir)\$($rUserInfo.SamAccountName)_$($rSrcPC.Name)_$($rTrgPC.Name)"
